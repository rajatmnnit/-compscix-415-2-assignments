...
`SalePrice` for houses with `ExterQualGd`(Good exterior material quality) are on average $71,529.49 lower relative to the houses with `ExterQualEx`(Excellent) while controlling for all other features.
...
This model has the adjusted-R-squared value of 0.69 which is lower than the adjusted-R-squared value of 0.78 achieved with the model we fit in HW 7 with `GrLivArea`, `OverallQual`, `Neighborhood`. It means that the current model explains less variability in the data as compared to the one built in HW 7. Hence, the model from HW 7 was a better fitting model.
\pagebreak
